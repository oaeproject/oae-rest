...
const createGroup = (restCtx, displayName, description, visibility, joinable, managers, members, callback) => {
  ...
  doRequest(restCtx, '/api/group/create', HTTP_POST, postData, parseResponse(callback));
};
...
const deleteGroup = (restCtx, groupId, callback) =>
  doRequest(restCtx, `/api/group/${encodeURIComponent(groupId)}`, HTTP_DELETE, null, parseResponse(callback));
...
const restoreGroup = (restCtx, groupId, callback) =>
  doRequest(restCtx, `/api/group/${encodeURIComponent(groupId)}/restore`, HTTP_POST, null, parseResponse(callback));
...
const getGroup = (restCtx, groupId, callback) => {
  doRequest(restCtx, `/api/group/${encodeURIComponent(groupId)}`, HTTP_GET, null, parseResponse(callback));
};
...
const updateGroup = (restCtx, groupId, profileFields, callback) => {
  doRequest(restCtx, `/api/group/${encodeURIComponent(groupId)}`, HTTP_POST, profileFields, parseResponse(callback));
};
...
const getGroupMembers = (restCtx, groupId, start, limit, callback) => {
  ...
  doRequest(
    restCtx,
    `/api/group/${encodeURIComponent(groupId)}/members`,
    HTTP_GET,
    parameters,
...
const setGroupMembers = (restCtx, groupId, members, callback) => {
  doRequest(restCtx, `/api/group/${encodeURIComponent(groupId)}/members`, HTTP_POST, members, parseResponse(callback));
};
...
const joinGroup = (restCtx, groupId, callback) => {
  doRequest(restCtx, `/api/group/${encodeURIComponent(groupId)}/join`, HTTP_POST, null, parseResponse(callback));
};
...
const leaveGroup = (restCtx, groupId, callback) => {
  doRequest(restCtx, `/api/group/${encodeURIComponent(groupId)}/leave`, HTTP_POST, null, parseResponse(callback));
};
...
const getMembershipsLibrary = (restCtx, userId, start, limit, callback) => {
  ...
  doRequest(
    restCtx,
    `/api/user/${encodeURIComponent(userId)}/memberships`,
    HTTP_GET,
    parameters,
    parseResponse(callback)
  );
...
const uploadPicture = (restCtx, groupId, file, selectedArea, callback) => {
  ...
  if (isDefined(selectedArea)) {
    doRequest(restCtx, `/api/group/${encodeURIComponent(groupId)}/picture`, HTTP_POST, parameters, (error) => {
      if (error) return callback(error);

      CropAPI.cropPicture(restCtx, groupId, selectedArea, parseResponse(callback));
    });
  } else {
    doRequest(
      restCtx,
      `/api/group/${encodeURIComponent(groupId)}/picture`,
      HTTP_POST,
      parameters,
      parseResponse(callback)
    );
...
const downloadPicture = (restCtx, groupId, size, callback) => {
  ...
  doRequest(restCtx, `/api/group/${encodeURIComponent(groupId)}`, HTTP_GET, null, (error, group) => {
    if (error) return callback(error);
...
const createRequestJoinGroup = (restCtx, groupId, callback) => {
  doRequest(
    restCtx,
    `/api/group/${encodeURIComponent(groupId)}/join-request`,
    HTTP_POST,
    null,
    parseResponse(callback)
  );
...
const getJoinGroupRequest = (restCtx, groupId, callback) => {
  doRequest(
    restCtx,
    `/api/group/${encodeURIComponent(groupId)}/join-request/mine`,
    HTTP_GET,
    null,
    parseResponse(callback)
  );
...
const getJoinGroupRequests = (restCtx, groupId, callback) => {
  ...
  doRequest(
    restCtx,
    `/api/group/${encodeURIComponent(groupId)}/join-request/all`,
    HTTP_GET,
    parameters,
    parseResponse(callback)
  );
