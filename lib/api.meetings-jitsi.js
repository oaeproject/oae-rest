...
const API_ENDPOINT = '/api/meeting-jitsi';
...
const createMeeting = (
  ...
  doRequest(restCtx, `${API_ENDPOINT}/create`, HTTP_POST, parameters, parseResponse(callback));
};
 ...
 */
const getMembers = (restCtx, meetingId, start, limit, callback) => {
  const parameters = {
    start,
    limit
  };
  doRequest(
    restCtx,
    `${API_ENDPOINT}/${encodeURIComponent(meetingId)}/members`,
    HTTP_GET,
    parameters,
 ...
 */
const updateMeeting = (restCtx, meetingId, parameters, callback) => {
  doRequest(restCtx, `${API_ENDPOINT}/${encodeURIComponent(meetingId)}`, HTTP_PUT, parameters, parseResponse(callback));
};
 ...
 * @param {Object}          callback.err        Error object containing error code and error message
 */
const deleteMeeting = (restCtx, meetingId, callback) => {
  doRequest(restCtx, `${API_ENDPOINT}/${encodeURIComponent(meetingId)}`, HTTP_DELETE, null, parseResponse(callback));
};
 ...
 * @param {Meeting}         callback.meeting    The updated meeting object
 */
const getMeeting = (restCtx, meetingId, callback) => {
  doRequest(restCtx, `${API_ENDPOINT}/${encodeURIComponent(meetingId)}`, HTTP_GET, null, parseResponse(callback));
};
 ...
 * @param {Meeting[]}       callback.meeting    The updated meeting object
 */
const getMeetingsLibrary = (restCtx, principalId, callback) => {
  doRequest(
    restCtx,
    `${API_ENDPOINT}/library/${encodeURIComponent(principalId)}`,
    HTTP_GET,
    null,
    parseResponse(callback)
  );
 ...
 * @param {Object}          callback.err        Error object containing error code and error message
 */
const updateMembers = (restCtx, meetingId, updatedMembers, callback) => {
  doRequest(
    restCtx,
    `${API_ENDPOINT}/${encodeURIComponent(meetingId)}/members`,
    HTTP_PUT,
 ...
 */
const createComment = (restCtx, meetingId, body, replyTo, callback) => {
  doRequest(
    restCtx,
    `${API_ENDPOINT}/${encodeURIComponent(meetingId)}/messages`,
    HTTP_POST,
    {
      body,
      replyTo
 ...
 */
const deleteComment = (restCtx, meetingId, created, callback) => {
  doRequest(
    restCtx,
    `${API_ENDPOINT}/${encodeURIComponent(meetingId)}/messages/${encodeURIComponent(created)}`,
    HTTP_DELETE,
    null,
    parseResponse(callback)
  );
