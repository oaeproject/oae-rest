 ...
 */
const encodeURI = function(uriComponent) {
  ...
  const req = request(opts, (err, response, body) => {
    if (err) {
      ...
      return callback({
        code: 500,
        msg: util.format(
          'Something went wrong trying to contact the server:\n%s\n%s',
          err.message,
          err.stack
        )
      });
 ...
 * @api private
 */
const _performRestRequest = function(...args) {
  ...
  if (restCtx.hostHeader) {
    ...
    const [protocol] = restCtx.host.split(':');
