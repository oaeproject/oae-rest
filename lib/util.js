 ...
 */
const doRequest = (opts, data, callback) => {
  data = defaultTo({}, data);
  ...
  if (isNotEmpty(data)) {
    if (equals(opts.method, HTTP_GET)) {
      opts.qs = data;
    } else if (and(isNotDefined(hasStream), differs(opts.method, HTTP_GET))) {
      opts.form = data;
    }
  }
...
const _performRestRequest = (...args) => {
  ...
  if (isObject(restCtx.additionalHeaders)) {
    requestOpts.headers = mergeAll([requestOpts.headers, restCtx.additionalHeaders]);
  }
  ...
  return doRequest(requestOpts, data, callback);
};
...
const performRestRequest = (...args) => {
  ...
  if (restCtx.cookieJar) return _performRestRequest(restCtx, url, method, data, callback);
