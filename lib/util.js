...
const promiseToRequest = promisify(request);
 ...
 */
const doRequest = async (options, data) => {
  data = defaultTo({}, data);
  ...
  if (isNotEmpty(data)) {
    const notStreamingData = isNotDefined(hasStream);
    const notAGETRequest = differs(options.method, HTTP_GET);
    if (equals(options.method, HTTP_GET)) {
      options.qs = data;
    } else if (and(notStreamingData, notAGETRequest)) {
      options.form = data;
    }
  }
    ...
    forEachObjIndexed((value, key) => {
      ...
      if (isArray(value)) {
        forEach((innerValue) => {
          ...
          if (isBoolean(value)) {
            formData.append(key, toString(innerValue));
          } else {
  ...
  const httpData = {};
  return promiseToRequest(options)
    .then((response) => {
      let { body } = response;
      httpData.body = body;
      httpData.response = response;

      if (isBelow400(response.statusCode)) {
        throw new Error(JSON.stringify({ statusCode: response.statusCode }));
      }
    ...
    .catch((error) => {
      ...
      if (error.statusCode) {
        const error = new Error(JSON.stringify({ code: httpData.response.statusCode, msg: httpData.body }));
        emitter.emit('error', error, httpData.body, httpData.response);
        throw error;
      }
      ...
      throw new Error(
        JSON.stringify({
          code: 500,
          msg: format('Something went wrong trying to contact the server:\n%s\n%s', error.message, error.stack)
        })
      );
    });
};
...
/**
 * Internal Function that will perform a REST request.
 * If no user is provided, the request will be done anonymously
 *
...
const _performRestRequest = (...args) => {
  const [restCtx, url, method, data] = args;
  ...
  if (restCtx.hostHeader) {
    ...
    referer = format('%s://%s/', protocol, restCtx.hostHeader);
  }
  ...
  return doRequest(requestOptions, data);
};
 ...
 */
const fillCookieJar = (restCtx) => {
  // If no user is specified, there is no point in doing a login request.
  if (isNotDefined(restCtx.username)) return;

  // Log the user in
  return _performRestRequest(restCtx, '/api/auth/login', HTTP_POST, {
...
const performRestRequest = async (...args) => {
  ...
  await fillCookieJar(restCtx);

  return _performRestRequest(restCtx, url, method, data);
};

const parseResponse = function (callback) {
...
export {
  parseResponse,
  encodeURI as encodeURIComponent,
  performRestRequest,
  fillCookieJar,
