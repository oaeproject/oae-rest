...
const createLink = (restCtx, linkDetails, callback) => {
  ...
  RestUtil.performRestRequest(restCtx, '/api/content/create', HTTP_POST, params, callback);
};
...
const removeContentFromLibrary = (restCtx, principalId, contentId, callback) => {
  const url = `/api/content/library/${RestUtil.encodeURIComponent(principalId)}/${RestUtil.encodeURIComponent(
    contentId
  )}`;
  RestUtil.performRestRequest(restCtx, url, HTTP_DELETE, null, callback);
};
...
const getRevision = (restCtx, contentId, revisionId, callback) => {
  const url = `/api/content/${RestUtil.encodeURIComponent(contentId)}/revisions/${RestUtil.encodeURIComponent(
    revisionId
  )}`;
...
const restoreRevision = (restCtx, contentId, revisionId, callback) => {
  ...
  RestUtil.performRestRequest(restCtx, url, HTTP_POST, null, callback);
};
...
const setPreviewItems = (
  ...
  const url =
    ...
    '/previews';
  RestUtil.performRestRequest(restCtx, url, HTTP_POST, params, callback);
};
...
const downloadPreviewItem = (restCtx, contentId, revisionId, previewItem, signature, callback) => {
  ...
  RestUtil.performRestRequest(restCtx, url, HTTP_GET, params, callback);
};
