...
const getContent = (restCtx, contentId, callback) => {
  RestUtil.performRestRequest(restCtx, '/api/content/' + RestUtil.encodeURIComponent(contentId), 'GET', null, callback);
};
 ...
 */
const createLink = (restCtx, displayName, description, visibility, link, managers, viewers, folders, callback) => {
  const params = {
  ...
  RestUtil.performRestRequest(restCtx, '/api/content/create', 'POST', params, callback);
};
 ...
 */
const updateContent = (restCtx, contentId, params, callback) => {
  RestUtil.performRestRequest(
    restCtx,
    '/api/content/' + RestUtil.encodeURIComponent(contentId),
    'POST',
    params,
    callback
  );
...
const getMembers = (restCtx, contentId, start, limit, callback) => {
  ...
  RestUtil.performRestRequest(
    restCtx,
    '/api/content/' + RestUtil.encodeURIComponent(contentId) + '/members',
    'GET',
    params,
    callback
  );
...
const shareContent = (restCtx, contentId, principals, callback) => {
  RestUtil.performRestRequest(
    restCtx,
    '/api/content/' + RestUtil.encodeURIComponent(contentId) + '/share',
    'POST',
    { viewers: principals },
    callback
  );
...
const createComment = (restCtx, contentId, body, replyTo, callback) => {
  RestUtil.performRestRequest(
    restCtx,
    '/api/content/' + RestUtil.encodeURIComponent(contentId) + '/messages',
    'POST',
    { body, replyTo },
    callback
  );
...
const deleteComment = (restCtx, contentId, created, callback) => {
  RestUtil.performRestRequest(
    restCtx,
    '/api/content/' + RestUtil.encodeURIComponent(contentId) + '/messages/' + RestUtil.encodeURIComponent(created),
    'DELETE',
    null,
    callback
  );
...
const getLibrary = (restCtx, principalId, start, limit, callback) => {
  ...
  RestUtil.performRestRequest(
    restCtx,
    '/api/content/library/' + RestUtil.encodeURIComponent(principalId),
    'GET',
    params,
    callback
  );
...
const removeContentFromLibrary = (restCtx, principalId, contentId, callback) => {
  const url =
    '/api/content/library/' + RestUtil.encodeURIComponent(principalId) + '/' + RestUtil.encodeURIComponent(contentId);
  RestUtil.performRestRequest(restCtx, url, 'DELETE', null, callback);
};
...
const getRevisions = (restCtx, contentId, start, limit, callback) => {
  ...
  RestUtil.performRestRequest(
    restCtx,
    '/api/content/' + RestUtil.encodeURIComponent(contentId) + '/revisions',
    'GET',
    params,
    callback
  );
...
const getRevision = (restCtx, contentId, revisionId, callback) => {
  const url =
    '/api/content/' + RestUtil.encodeURIComponent(contentId) + '/revisions/' + RestUtil.encodeURIComponent(revisionId);
...
const restoreRevision = (restCtx, contentId, revisionId, callback) => {
  const url =
    ...
    '/restore';
  RestUtil.performRestRequest(restCtx, url, 'POST', null, callback);
};
...
const updateFileBody = (restCtx, contentId, file, callback) => {
  ...
  RestUtil.performRestRequest(
    restCtx,
    '/api/content/' + RestUtil.encodeURIComponent(contentId) + '/newversion',
    'POST',
    params,
    callback
  );
...
const download = (restCtx, contentId, revisionId, path, callback) => {
  ...
  const downloadFile = function() {
    let url = restCtx.host + '/api/content/' + RestUtil.encodeURIComponent(contentId) + '/download';
    if (revisionId) {
      url += '/' + revisionId;
    }
    ...
    req.on('end', () => {
      ...
      if (![200, 204].includes(response.statusCode) && !called) {
        called = true;
        callback({
  ...
  // We can't use the RestUtil.performRestRequest utility to wrap our requests as we're dealing with streams.
  ...
  if (restCtx.cookieJar) {
    ...
    RestUtil.fillCookieJar(restCtx, err => {
      if (err) {
        return callback(err);
      }
...
const joinCollabDoc = (restCtx, contentId, callback) => {
  RestUtil.performRestRequest(
    restCtx,
    '/api/content/' + RestUtil.encodeURIComponent(contentId) + '/join',
    'POST',
    null,
    callback
  );
...
const setPreviewItems = (
  ...
  const url =
    ...
    RestUtil.encodeURIComponent(revisionId) +
    '/previews';
  RestUtil.performRestRequest(restCtx, url, 'POST', params, callback);
};
...
const getPreviewItems = (restCtx, contentId, revisionId, callback) => {
  const url =
    ...
    RestUtil.encodeURIComponent(revisionId) +
    '/previews';
  RestUtil.performRestRequest(restCtx, url, 'GET', {}, callback);
};
...
const downloadPreviewItem = (restCtx, contentId, revisionId, previewItem, signature, callback) => {
  ...
  RestUtil.performRestRequest(restCtx, url, 'GET', params, callback);
};
