...
const getModules = (restCtx, type, callback) => {
  RestUtil.performRestRequest(restCtx, `/api/doc/${RestUtil.encodeURIComponent(type)}`, HTTP_GET, null, callback);
};
...
const getModuleDocumentation = (restCtx, type, moduleId, callback) => {
  RestUtil.performRestRequest(
    restCtx,
    `/api/doc/${RestUtil.encodeURIComponent(type)}/${RestUtil.encodeURIComponent(moduleId)}`,
    HTTP_GET,
    null,
    callback
  );
 ...
 */
const getSwaggerResources = (restCtx, callback) => {
  RestUtil.performRestRequest(restCtx, `/api/swagger`, HTTP_GET, null, callback);
};
 ...
 */
const getSwaggerApi = (restCtx, id, callback) => {
  RestUtil.performRestRequest(restCtx, `/api/swagger/${id}`, HTTP_GET, null, callback);
};

export { getModules, getModuleDocumentation, getSwaggerResources, getSwaggerApi };
