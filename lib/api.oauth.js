...
const createClient = (restCtx, userId, displayName, callback) => {
  doRequest(
    restCtx,
    `/api/auth/oauth/clients/${encodeURIComponent(userId)}`,
    HTTP_POST,
    { displayName },
...
const getClients = (restCtx, userId, callback) => {
  doRequest(restCtx, `/api/auth/oauth/clients/${encodeURIComponent(userId)}`, HTTP_GET, null, parseResponse(callback));
};
...
const updateClient = (restCtx, userId, clientId, displayName, secret, callback) => {
  const url = `/api/auth/oauth/clients/${encodeURIComponent(userId)}/${encodeURIComponent(clientId)}`;
  const parameters = {
    displayName,
    secret
  };
...
const deleteClient = (restCtx, userId, clientId, callback) => {
  const url = `/api/auth/oauth/clients/${encodeURIComponent(userId)}/${encodeURIComponent(clientId)}`;
  doRequest(restCtx, url, 'DELETE', null, parseResponse(callback));
};
