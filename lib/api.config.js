...
const getSchema = (restCtx, callback) => {
  doRequest(restCtx, '/api/config/schema', HTTP_GET, null, parseResponse(callback));
};
...
const updateConfig = (restCtx, tenantAlias, update, callback) => {
  ...
  doRequest(restCtx, url, HTTP_POST, update, parseResponse(callback));
};
...
const clearConfig = (restCtx, tenantAlias, configFields, callback) => {
  ...
  url += '/clear';
  doRequest(restCtx, url, HTTP_POST, { configFields }, parseResponse(callback));
};
