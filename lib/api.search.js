...
const search = (restCtx, searchType, parameters, options, callback) => {
  parameters = defaultToEmptyArray(parameters);
  options = defaultToEmptyObject(options);

  // Url-encode and join the path parameters into a path string
  parameters = map((parameter) => encodeURIComponent(parameter), parameters);
  parameters = preffixSlash(parameters);

  let path = `/api/search/${encodeURIComponent(searchType)}`;
  if (parameters) {
    path += `/${parameters}`;
  }

  doRequest(restCtx, path, HTTP_GET, options, parseResponse(callback));
};
...
const refresh = (restCtx, callback) => {
  doRequest(restCtx, '/api/search/_refresh', HTTP_POST, null, parseResponse(callback));
};
...
const reindexAll = (globalAdminRestCtx, callback) => {
  doRequest(globalAdminRestCtx, '/api/search/reindexAll', HTTP_POST, null, parseResponse(callback));
};
