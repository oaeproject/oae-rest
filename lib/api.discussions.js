...
const createDiscussion = (restCtx, displayName, description, visibility, managers, members, callback) => {
  ...
  RestUtil.performRestRequest(restCtx, `/api/discussion/create`, HTTP_POST, parameters, callback);
};
...
const getDiscussion = (restCtx, discussionId, callback) => {
  RestUtil.performRestRequest(
    restCtx,
    `/api/discussion/${RestUtil.encodeURIComponent(discussionId)}`,
    HTTP_GET,
    null,
    callback
  );
...
const getDiscussionsLibrary = (restCtx, principalId, start, limit, callback) => {
  ...
  RestUtil.performRestRequest(
    restCtx,
    `/api/discussion/library/${RestUtil.encodeURIComponent(principalId)}`,
    HTTP_GET,
    parameters,
    callback
  );
...
const getDiscussionMembers = (restCtx, discussionId, start, limit, callback) => {
  ...
  RestUtil.performRestRequest(
    restCtx,
    `/api/discussion/${RestUtil.encodeURIComponent(discussionId)}/members`,
    HTTP_GET,
    parameters,
    callback
  );
...
const shareDiscussion = (restCtx, discussionId, principalIds, callback) => {
  ...
  RestUtil.performRestRequest(
    restCtx,
    `/api/discussion/${RestUtil.encodeURIComponent(discussionId)}/share`,
    HTTP_POST,
    parameters,
    callback
  );
...
const removeDiscussionFromLibrary = (restCtx, libraryOwnerId, discussionId, callback) => {
  RestUtil.performRestRequest(
    restCtx,
    `/api/discussion/library/${RestUtil.encodeURIComponent(libraryOwnerId)}/${RestUtil.encodeURIComponent(
      discussionId
    )}`,
    HTTP_DELETE,
    null,
...
const createMessage = (restCtx, discussionId, body, replyTo, callback) => {
  ...
  RestUtil.performRestRequest(
    restCtx,
    `/api/discussion/${RestUtil.encodeURIComponent(discussionId)}/messages`,
    HTTP_POST,
    parameters,
    callback
  );
...
const deleteMessage = (restCtx, discussionId, messageCreated, callback) => {
  RestUtil.performRestRequest(
    restCtx,
    `/api/discussion/${RestUtil.encodeURIComponent(discussionId)}/messages/${RestUtil.encodeURIComponent(
      messageCreated
    )}`,
    HTTP_DELETE,
    null,
