 ...
 */
const createDiscussion = (restCtx, displayName, description, visibility, managers, members, callback) => {
  const parameters = {
    displayName,
    description,
    visibility,
  ...
  doRequest(restCtx, `/api/discussion/create`, HTTP_POST, parameters, parseResponse(callback));
};
 ...
 */
const getDiscussion = (restCtx, discussionId, callback) => {
  doRequest(restCtx, `/api/discussion/${encodeURIComponent(discussionId)}`, HTTP_GET, null, parseResponse(callback));
};
 ...
 */
const updateDiscussion = (restCtx, discussionId, profileFields, callback) => {
  doRequest(
    restCtx,
    `/api/discussion/${encodeURIComponent(discussionId)}`,
    HTTP_POST,
    profileFields,
 ...
 * @param  {Object}         callback.err                    An error that occurred, if any
 */
const deleteDiscussion = (restCtx, discussionId, callback) => {
  doRequest(restCtx, `/api/discussion/${encodeURIComponent(discussionId)}`, HTTP_DELETE, null, parseResponse(callback));
};
 ...
 */
const getDiscussionsLibrary = (restCtx, principalId, start, limit, callback) => {
  const parameters = {
    start,
    limit
  };
  doRequest(restCtx, `/api/discussion/library/${encodeURIComponent(principalId)}`, HTTP_GET, parameters, parseResponse(callback));
};
 ...
 */
const getDiscussionMembers = (restCtx, discussionId, start, limit, callback) => {
  const parameters = {
    start,
    limit
  };
  doRequest(restCtx, `/api/discussion/${encodeURIComponent(discussionId)}/members`, HTTP_GET, parameters, parseResponse(callback));
};
 ...
 * @param  {Object}         callback.err            An error that occurred, if any
 */
const updateDiscussionMembers = (restCtx, discussionId, memberUpdates, callback) => {
  doRequest(restCtx, `/api/discussion/${encodeURIComponent(discussionId)}/members`, HTTP_POST, memberUpdates, parseResponse(callback));
};
 ...
 * @param  {Object}         callback.err    An error that occurred, if any
 */
const shareDiscussion = (restCtx, discussionId, principalIds, callback) => {
  const parameters = {
    members: principalIds
  };
  doRequest(restCtx, `/api/discussion/${encodeURIComponent(discussionId)}/share`, HTTP_POST, parameters, parseResponse(callback));
};
 ...
 * @param  {Object}         callback.err    An error that occurred, if any
 */
const removeDiscussionFromLibrary = (restCtx, libraryOwnerId, discussionId, callback) => {
  doRequest(
    restCtx,
    `/api/discussion/library/${encodeURIComponent(libraryOwnerId)}/${encodeURIComponent(discussionId)}`,
    HTTP_DELETE,
    null,
    parseResponse(callback)
  );
 ...
 */
const createMessage = (restCtx, discussionId, body, replyTo, callback) => {
  const parameters = {
    body,
    replyTo
  };
  doRequest(restCtx, `/api/discussion/${encodeURIComponent(discussionId)}/messages`, HTTP_POST, parameters, parseResponse(callback));
};
 ...
 */
const getMessages = (restCtx, discussionId, start, limit, callback) => {
  const parameters = {
    start,
    limit
  };
  doRequest(restCtx, `/api/discussion/${encodeURIComponent(discussionId)}/messages`, HTTP_GET, parameters, parseResponse(callback));
};
 ...
 */
const deleteMessage = (restCtx, discussionId, messageCreated, callback) => {
  doRequest(
    restCtx,
    `/api/discussion/${encodeURIComponent(discussionId)}/messages/${encodeURIComponent(messageCreated)}`,
    HTTP_DELETE,
    null,
    parseResponse(callback)
  );
