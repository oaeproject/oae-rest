...
const createDiscussion = (restCtx, displayName, description, visibility, managers, members, callback) => {
  ...
  RestUtil.performRestRequest(restCtx, '/api/discussion/create', 'POST', params, callback);
};
...
const removeDiscussionFromLibrary = (restCtx, libraryOwnerId, discussionId, callback) => {
  RestUtil.performRestRequest(
    restCtx,
    '/api/discussion/library/' +
      RestUtil.encodeURIComponent(libraryOwnerId) +
      '/' +
      RestUtil.encodeURIComponent(discussionId),
    'DELETE',
    null,
    callback
  );
...
const deleteMessage = (restCtx, discussionId, messageCreated, callback) => {
  RestUtil.performRestRequest(
    ...
    '/api/discussion/' +
      RestUtil.encodeURIComponent(discussionId) +
      '/messages/' +
      RestUtil.encodeURIComponent(messageCreated),
    'DELETE',
    null,
    callback
  );
