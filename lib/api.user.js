...
const createGlobalAdminUser = (restCtx, username, password, displayName, email, opts, callback) => {
  ...
  RestUtil.performRestRequest(restCtx, '/api/user/createGlobalAdminUser', HTTP_POST, opts, callback);
};
...
const _createTenantAdminUser = (restCtx, tenantAlias, ...args) => {
  let [username, password, displayName, email, opts, callback] = args;
  opts = defaultTo({}, opts);
...
const updateUser = (restCtx, userId, params, callback) => {
  RestUtil.performRestRequest(restCtx, `/api/user/${RestUtil.encodeURIComponent(userId)}`, HTTP_POST, params, callback);
};
