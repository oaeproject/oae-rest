...
const createGlobalAdminUser = (restCtx, username, password, displayName, email, options, callback) => {
  ...
  doRequest(restCtx, '/api/user/createGlobalAdminUser', HTTP_POST, options, parseResponse(callback));
};
...
const _createTenantAdminUser = (restCtx, tenantAlias, ...args) => {
  ...
  if (tenantAlias) {
    url = `/api/user/${encodeURIComponent(tenantAlias)}/createTenantAdminUser`;
  }
...
const _createUser = (restCtx, tenantAlias, ...args) => {
  ...
  if (tenantAlias) {
    url = `/api/user/${encodeURIComponent(tenantAlias)}/create`;
  }
...
const deleteUser = (restCtx, userId, callback) => {
  doRequest(restCtx, `/api/user/${encodeURIComponent(userId)}`, HTTP_DELETE, null, parseResponse(callback));
};
...
const restoreUser = (restCtx, userId, callback) => {
  doRequest(restCtx, `/api/user/${encodeURIComponent(userId)}/restore`, HTTP_POST, null, parseResponse(callback));
};
...
const getMe = (restCtx, callback) => {
  doRequest(restCtx, '/api/me', HTTP_GET, null, parseResponse(callback));
};
...
const getUser = (restCtx, userId, callback) => {
  doRequest(restCtx, `/api/user/${encodeURIComponent(userId)}`, HTTP_GET, null, parseResponse(callback));
};
...
const updateUser = (restCtx, userId, parameters, callback) => {
  doRequest(restCtx, `/api/user/${encodeURIComponent(userId)}`, HTTP_POST, parameters, parseResponse(callback));
};
...
const uploadPicture = (restCtx, userId, file, selectedArea, callback) => {
  ...
  if (isDefined(selectedArea)) {
    doRequest(restCtx, `/api/user/${encodeURIComponent(userId)}/picture`, HTTP_POST, parameters, (error) => {
      if (error) return callback(error);

      CropAPI.cropPicture(restCtx, userId, selectedArea, parseResponse(callback));
    });
  } else {
    doRequest(
      restCtx,
      `/api/user/${encodeURIComponent(userId)}/picture`,
      HTTP_POST,
      parameters,
...
const downloadPicture = (restCtx, userId, size, callback) => {
  ...
  getUser(restCtx, userId, (error, user) => {
    if (error) return callback(error);
...
const setTenantAdmin = (restCtx, userId, value, callback) => {
  doRequest(
    restCtx,
    `/api/user/${encodeURIComponent(userId)}/admin`,
    HTTP_POST,
    { admin: isTrue(value) },
    parseResponse(callback)
  );
...
const getTimezones = (restCtx, callback) => {
  doRequest(restCtx, '/api/timezones', HTTP_GET, null, parseResponse(callback));
};
...
const getTermsAndConditions = (restCtx, locale, callback) => {
  doRequest(restCtx, '/api/user/termsAndConditions', HTTP_GET, { locale }, parseResponse(callback));
};
...
const acceptTermsAndConditions = (restCtx, userId, callback) => {
  const url = `/api/user/${encodeURIComponent(userId)}/termsAndConditions`;
  doRequest(restCtx, url, HTTP_POST, {}, parseResponse(callback));
};
...
const verifyEmail = (restCtx, userId, token, callback) => {
  const url = `/api/user/${encodeURIComponent(userId)}/email/verify`;
  doRequest(restCtx, url, HTTP_POST, { token }, parseResponse(callback));
};
...
const resendEmailToken = (restCtx, userId, callback) => {
  const url = `/api/user/${encodeURIComponent(userId)}/email/resend`;
  doRequest(restCtx, url, HTTP_POST, {}, parseResponse(callback));
};
...
const getEmailToken = (restCtx, userId, callback) => {
  const url = `/api/user/${encodeURIComponent(userId)}/email/token`;
...
const deleteEmailToken = (restCtx, userId, callback) => {
  const url = `/api/user/${encodeURIComponent(userId)}/email/token`;
  doRequest(restCtx, url, HTTP_DELETE, {}, parseResponse(callback));
};
...
const getRecentlyVisitedGroups = (restCtx, userId, callback) => {
  const url = `/api/user/${encodeURIComponent(userId)}/groups/recent`;
  doRequest(restCtx, url, HTTP_GET, {}, parseResponse(callback));
};
...
const exportPersonalData = (restCtx, userId, exportType, callback) => {
  const url = `/api/user/${encodeURIComponent(userId)}/export/${encodeURIComponent(exportType)}`;
  doRequest(restCtx, url, HTTP_GET, {}, parseResponse(callback));
};
