...
const createTenantNetwork = (restCtx, displayName, callback) => {
  RestUtil.RestRequest(restCtx, '/api/tenantNetwork/create', 'POST', { displayName }, callback);
};
...
const getTenantsByEmailAddress = (restCtx, emails, callback) => {
  RestUtil.RestRequest(restCtx, '/api/tenantsByEmail', 'GET', { emails }, callback);
};
...
const createTenant = (restCtx, alias, displayName, host, opts, callback) => {
  ...
  RestUtil.RestRequest(restCtx, '/api/tenant/create', 'POST', params, (err, tenant) => {
    if (err) {
      return callback(err);
    }
...
const updateTenant = (restCtx, alias, tenantUpdates, callback) => {
  ...
  RestUtil.RestRequest(restCtx, url, 'POST', params, err => {
    if (err) {
      callback(err);
    } else {
...
const stopTenant = (restCtx, alias, callback) => {
  RestUtil.RestRequest(restCtx, '/api/tenant/stop', 'POST', { aliases: [alias] }, err => {
    if (err) {
      callback(err);
    } else {
...
const startTenant = (restCtx, tenantAlias, callback) => {
  RestUtil.RestRequest(restCtx, '/api/tenant/start', 'POST', { aliases: [tenantAlias] }, err => {
    if (err) {
      callback(err);
    } else {
