...
/**
 * Stopping a server is async, this variable holds how long we should wait
 * before returning on start/stop/delete of a tenant
 */
const WAIT_TIME = 100;
...
const createTenantNetwork = (restCtx, displayName, callback) => {
  doRequest(restCtx, '/api/tenantNetwork/create', HTTP_POST, { displayName }, parseResponse(callback));
};
...
const getTenantNetworks = (restCtx, callback) => {
  doRequest(restCtx, '/api/tenantNetworks', HTTP_GET, null, parseResponse(callback));
};
...
const updateTenantNetwork = (restCtx, id, displayName, callback) => {
  doRequest(
    restCtx,
    `/api/tenantNetwork/${encodeURIComponent(id)}`,
    HTTP_POST,
    { displayName },
...
const deleteTenantNetwork = (restCtx, id, callback) => {
  doRequest(restCtx, `/api/tenantNetwork/${encodeURIComponent(id)}`, HTTP_DELETE, null, parseResponse(callback));
};
...
const addTenantAliases = (restCtx, tenantNetworkId, tenantAliases, callback) => {
  doRequest(
    restCtx,
    `/api/tenantNetwork/${encodeURIComponent(tenantNetworkId)}/addTenants`,
    HTTP_POST,
    { alias: tenantAliases },
    parseResponse(callback)
  );
...
const removeTenantAliases = (restCtx, tenantNetworkId, tenantAliases, callback) => {
  doRequest(
    restCtx,
    `/api/tenantNetwork/${encodeURIComponent(tenantNetworkId)}/removeTenants`,
    HTTP_POST,
    { alias: tenantAliases },
    parseResponse(callback)
  );
...
const getTenants = (restCtx, callback) => {
  doRequest(restCtx, '/api/tenants', HTTP_GET, null, parseResponse(callback));
};
...
const getTenantsByEmailAddress = (restCtx, emails, callback) => {
  doRequest(restCtx, '/api/tenantsByEmail', HTTP_GET, { emails }, parseResponse(callback));
};
...
const createTenant = (restCtx, alias, displayName, host, options, callback) => {
  ...
  doRequest(restCtx, '/api/tenant/create', HTTP_POST, parameters, (error, response) => {
    if (error) return callback(error);

    // Give the tenant some time to start
    return setTimeout(callback, WAIT_TIME, error, JSON.parse(response.body));
  });
};
...
const updateTenant = (restCtx, alias, tenantUpdates, callback) => {
  tenantUpdates = defaultToEmptyObject(tenantUpdates);
  ...
  doRequest(restCtx, url, HTTP_POST, parameters, (error) => {
    if (error) return callback(JSON.parse(error.message));

    // Give it some time to update
    setTimeout(callback, WAIT_TIME, error);
  });
};
...
const stopTenant = (restCtx, alias, callback) => {
  doRequest(restCtx, '/api/tenant/stop', HTTP_POST, { aliases: [alias] }, (error) => {
    if (error) return callback(error);
...
const startTenant = (restCtx, tenantAlias, callback) => {
  doRequest(restCtx, '/api/tenant/start', HTTP_POST, { aliases: [tenantAlias] }, (error) => {
    if (error) return callback(error);
...
const getLandingPage = (restCtx, callback) => {
  doRequest(restCtx, '/api/tenant/landingPage', HTTP_GET, null, parseResponse(callback));
};
