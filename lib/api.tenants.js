...
const createTenant = (restCtx, alias, displayName, host, options, callback) => {
  ...
  RestUtil.performRestRequest(restCtx, '/api/tenant/create', HTTP_POST, parameters, (error, tenant) => {
    if (error) return callback(error);

    // Give the tenant some time to start
    return setTimeout(callback, WAIT_TIME, error, tenant);
  });
};
...
const updateTenant = (restCtx, alias, tenantUpdates, callback) => {
  ...
  RestUtil.performRestRequest(restCtx, url, HTTP_POST, parameters, (error) => {
    if (error) {
      callback(error);
    } else {
...
const stopTenant = (restCtx, alias, callback) => {
  RestUtil.performRestRequest(restCtx, '/api/tenant/stop', HTTP_POST, { aliases: [alias] }, (error) => {
    if (error) {
      callback(error);
    } else {
...
const startTenant = (restCtx, tenantAlias, callback) => {
  RestUtil.performRestRequest(restCtx, '/api/tenant/start', HTTP_POST, { aliases: [tenantAlias] }, (error) => {
    if (error) {
      callback(error);
    } else {
