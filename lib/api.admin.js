 ...
 * a global and/or tenant administrators. It's expected that the RestContext
 * objects that are passed into these methods reflect authenticated
 * users whom are all administrators
 */
...
const _doSignedAuthenticationWithRequestInfo = (requestInfo, internalBaseUrl, strictSSL, callback) => {
  ...
  const baseUrl = defaultTo(format('%s//%s', parsedUrl.protocol, parsedUrl.host), internalBaseUrl);
  const authenticatingRestCtx = new RestContext(baseUrl, {
    hostHeader: parsedUrl.host,
    strictSSL
  });
  doSignedAuthentication(authenticatingRestCtx, requestInfo.body, (error) => {
    if (error) return callback(error);
...
const loginAsUser = (adminRestCtx, becomeUserId, targetInternalBaseUrl, callback) => {
  getSignedBecomeUserAuthenticationRequestInfo(adminRestCtx, becomeUserId, (error, requestInfo) => {
    if (error) return callback(error);
...
const loginOnTenant = (globalAdminRestCtx, tenantAlias, targetInternalBaseUrl, callback) => {
  getSignedTenantAuthenticationRequestInfo(globalAdminRestCtx, tenantAlias, (error, requestInfo) => {
    if (error) return callback(error);
