...
const getSignedBecomeUserAuthenticationRequestInfo = (adminRestCtx, becomeUserId, callback) => {
  RestUtil.RestRequest(adminRestCtx, '/api/auth/signed/become', 'GET', { becomeUserId }, callback);
};
...
const doSignedAuthentication = (restCtx, body, callback) => {
  RestUtil.RestRequest(restCtx, '/api/auth/signed', 'POST', body, (err, body, response) => {
    if (err) {
      return callback(err);
    }
