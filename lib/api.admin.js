...
const { RestContext } = require('./model');
...
const doSignedAuthentication = (restCtx, body, callback) => {
  RestUtil.performRestRequest(restCtx, '/api/auth/signed', 'POST', body, (err, body, response) => {
    if (err) {
      return callback(err);
    }
 ...
 * @api private
 */
const _doSignedAuthenticationWithRequestInfo = (
  requestInfo,
  internalBaseUrl,
  strictSSL,
  callback
...
const importUsers = (
  ...
  RestUtil.performRestRequest(restCtx, '/api/user/import', 'POST', params, callback);
};
...
const getAllUsersForTenant = (adminRestCtx, tenantAlias, callback) => {
  const url = '/api/tenants/' + RestUtil.encodeURIComponent(tenantAlias) + '/users';
  RestUtil.performRestRequest(adminRestCtx, url, 'GET', {}, callback);
};
