...
const reprocessPreviews = (globalAdminRestContext, filters, callback) => {
  doRequest(globalAdminRestContext, '/api/content/reprocessPreviews', HTTP_POST, filters, parseResponse(callback));
};
...
const expandUrl = (restCtx, url, callback) => {
  ...
  doRequest(restCtx, '/api/longurl/expand', HTTP_GET, data, parseResponse(callback));
};
