 ...
 */
const reprocessPreview = (restCtx, contentId, revisionId, callback) => {
 ...
 */
const reprocessPreviews = (globalAdminRestContext, filters, callback) => {
  RestUtil.performRestRequest(globalAdminRestContext, '/api/content/reprocessPreviews', HTTP_POST, filters, callback);
};
 ...
 */
const expandUrl = (restCtx, url, callback) => {
  const data = {
    url: RestUtil.encodeURIComponent(url)
  };
  RestUtil.performRestRequest(restCtx, '/api/longurl/expand', HTTP_GET, data, callback);
};

export { reprocessPreview, reprocessPreviews, expandUrl };
