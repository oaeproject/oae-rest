...
const createLtiTool = (restCtx, groupId, url, secret, key, displayName, description, callback) => {
  ...
  doRequest(restCtx, `/api/lti/${encodeURIComponent(groupId)}/create`, HTTP_POST, postData, parseResponse(callback));
};
...
const deleteLtiTool = (restCtx, groupId, toolId, callback) =>
  doRequest(
    restCtx,
    `/api/lti/${encodeURIComponent(groupId)}/${encodeURIComponent(toolId)}`,
    HTTP_DELETE,
    null,
...
const getLtiTools = (restCtx, groupId, callback) => {
  doRequest(restCtx, `/api/lti/${encodeURIComponent(groupId)}`, HTTP_GET, null, parseResponse(callback));
};
